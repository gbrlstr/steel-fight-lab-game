...
export function createFightRuntime(app: HTMLElement, options: FightRuntimeOptions = {}) {
    ...
    const impacts = hitSparks(); const sparks = new Map<string, HitSpark>(); const seen = new Set<string>(); const pending: { id: string; kind: 'hit' | 'block'; x: number; face: number; hero: string; action: string }[] = []; const effects = projectileEffects(); const projectiles = new Map<string, ProjectileVisual>()
    const lastAction = ['', '']
    ...
    function cues() {
        state.fighters.forEach((f, i) => { if (f.action !== lastAction[i]) { lastAction[i] = f.action; if (engaged) sfx.swing(f.hero, f.action) } })
    ...
    function takeHits() { for (const e of state.events ?? []) if (!seen.has(e.id)) { seen.add(e.id); const kind = e.kind === 'block' ? 'block' : 'hit'; pending.push({ id: e.id, kind, x: e.x, face: e.face ?? 1, hero: e.hero ?? state.fighters[0].hero, action: e.action ?? '' }); sfx.impact(e.hero ?? state.fighters[0].hero, e.action ?? '', kind) } }
    function animate(time: number) {
        ...
        for (const e of pending) { const i = Math.abs(state.fighters[0].x - e.x) <= Math.abs(state.fighters[1].x - e.x) ? 0 : 1; const origin = models[i]?.root.position; const spark = impacts.create(e.kind, origin ? { x: origin.x, y: origin.y + 1.55 * FIGHTER_PRESENCE, z: origin.z + .55 * FIGHTER_PRESENCE } : { x: e.x / 300, y: 1.55 * FIGHTER_PRESENCE, z: 4.15 + .55 * FIGHTER_PRESENCE }, e.face, time); spark.root.scale.setScalar(FIGHTER_PRESENCE); tagFx(spark.root); actors.add(spark.root); sparks.set(e.id, spark) } pending.length = 0; for (const [id, spark] of sparks) if (!spark.update(time)) { actors.remove(spark.root); spark.dispose(); sparks.delete(id) } if (seen.size > 4096) seen.clear()
