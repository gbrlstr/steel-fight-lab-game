...
export function createFightRuntime(app: HTMLElement, options: FightRuntimeOptions = {}) {
    ...
    async function fighters() { if (introPlayed) sfx.stop(); introPlayed = false; engaged = false; matchCalled = false; if (matchIntro) { matchIntro.dispose(); matchIntro = null } lastAction[0] = lastAction[1] = ''; hideVersus(); hideRoundCall(); hideVictory(); const gen = ++generation; loaded = false; const skinOf = (i: number) => options.skins?.[i] ?? 'default'; if (!state.fighters.every((f, i) => fighterStocked(f.hero, FIGHTER_PRESENCE, skinOf(i)))) get('announcement').textContent = 'LOADING FIGHTERS'; try { const next = await Promise.all(state.fighters.map((f, i) => acquireFighter(f.hero, FIGHTER_PRESENCE, skinOf(i)))); if (disposed || gen !== generation) { next.forEach(releaseFighter); return } models.forEach(releaseFighter); models = next; models.forEach((m, i) => { tagHero(m.root); m.root.visible = false; m.root.position.set(i ? 8.05 : -8.05, 0, 4.15); actors.add(m.root) }); loaded = true; get('announcement').textContent = ''; const missing = next.flatMap(m => m.unmatched); get('status').textContent = missing.length ? `Accessories: ${missing.length} helper bones follow hierarchy (check)` : (network ? 'Online match • authoritative server' : 'Local fight • two controllers'); } catch (e) { get('announcement').textContent = 'LOAD FAILED'; get('status').textContent = String(e) } }
    function moveTable() {
    ...
    function cues() {
        state.fighters.forEach((f, i) => {
            ...
            if (f.hero === 'bristleback' && f.action.startsWith('QUILLSPRAY_')) {
                const tier = f.action.includes('FINISH') ? 1.25 : f.action.includes('4') ? 1.15 : f.action.includes('3') ? 1.08 : f.action.includes('2') ? 1.02 : 1
                const cast = quillFx.create({ x: f.x / 300, y: 1.45 * FIGHTER_PRESENCE, z: 4.15 + .55 * FIGHTER_PRESENCE }, f.face, performance.now(), tier)
                cast.root.scale.setScalar(FIGHTER_PRESENCE)
                tagFx(cast.root)
                actors.add(cast.root)
    ...
    function takeHits() {
        for (const e of state.events ?? []) {
            ...
            if (kind === 'swap' || kind === 'install' || e.action === 'SWAP_ACTION_DEFINITION' || e.action === 'LUMINOSITY_ACTION_DEFINITION' || e.action?.startsWith('STARBREAKER_') || e.action?.startsWith('QUILLSPRAY_')) continue
    ...
    function animate(time: number) {
        ...
        for (const e of pending) {
            ...
            if (e.kind === 'swap') {
                ...
                if (e.kind === 'hit' && e.hero === 'bristleback' && e.action?.startsWith('QUILLSPRAY_')) {
                    const tier = e.action.includes('FINISH') ? 1.2 : e.action.includes('4') ? 1.1 : 1
                    const burst = quillFx.create(at, e.face, time, tier)
                    burst.root.scale.setScalar(FIGHTER_PRESENCE * 0.85)
                    tagFx(burst.root)
                    actors.add(burst.root)
